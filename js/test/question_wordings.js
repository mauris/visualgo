...
const MST_PRIM_SEQUENCE = "Given the undirected weighted graph as shown in the picture, click the first |amt| edges in the sequence of edges that are added to the MST by Prim\'s algorithm starting at vertex |value|.";
const MST_KRUSKAL_SEQUENCE = "Given the undirected weighted graph as shown in the picture, click the first |amt| edges in the sequence of edges that are added to the MST by Kruskal\'s algorithm.";
...
const SSSP_GREATER_LESS = "Click all vertices (in any order) in this directed weighted graph that are reachable from source vertex |source| and have shortest path values |greaterless| than |value| from the source. Note that all edge weights are printed ";
